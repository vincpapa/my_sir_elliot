...
class RAPLT_Recall(BaseMetric):
    ...
    def __user_aplt(user_recommendations, cutoff, long_tail, relevant_items):
        ...
        """
        try:
            # if len(set([i for i, v in user_recommendations[:cutoff] if i in relevant_items]) & set(long_tail)) / max(min([len(user_recommendations[:cutoff]), len(set(relevant_items) & set(long_tail))]), 1) != 0:
            #     print('ok')
            return len(set([i for i, v in user_recommendations[:cutoff] if i in relevant_items]) & set(long_tail)) # / max(min([len(user_recommendations[:cutoff]), len(set(relevant_items) & set(long_tail))]), 1)
    ...
    def eval_user_metric(self):
        ...
        :return: the overall averaged value of APLT
        """
        return {u: RAPLT_Recall.__user_aplt(u_r, self._cutoff, self._long_tail, self._relevance.get_user_rel(u))
             for u, u_r in self._recommendations.items()}
